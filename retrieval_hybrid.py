...
def load_documents_for_bm25(limit: Optional[int] = None) -> list[Document]:
    ...
    try:
        ...
        for doc in cursor:
            ...
            if "owners" in doc:
                metadata["owners"] = doc["owners"]
            if "last_edited_by" in doc:
                metadata["last_edited_by"] = doc["last_edited_by"]
            if "related_ticket" in doc:
                metadata["related_ticket"] = doc["related_ticket"]
            if "module" in doc:
                metadata["module"] = doc["module"]
            if "file_type" in doc:
...
def retrieve_and_compare(
    ...
    return {
        ...
        "bm25": {
            ...
            "sources": [f"{d.metadata.get('title', 'Unknown')} ({d.metadata.get('area', 'N/A')})" for d in bm25_results]
        },
        "vector": {
            "documents": vector_results,
            "sources": [f"{d.metadata.get('title', 'Unknown')} ({d.metadata.get('area', 'N/A')})" for d in vector_results]
        },
        "hybrid": {
            "documents": hybrid_results,
            "sources": [f"{d.metadata.get('title', 'Unknown')} ({d.metadata.get('area', 'N/A')})" for d in hybrid_results]
        }
    }
...
def main():
    ...
    # Test queries - based on actual engineering docs
    test_queries = [
        {
            "query": "How do I access the RDS production database?",
            "expected_strength": "BM25 (specific terms: RDS, database)",
        },
        {
            "query": "What's the process for enabling and using feature flags?",
            "expected_strength": "Vector (semantic: process, enabling, using)",
        },
        {
            "query": "authentication flow two factor",
            "expected_strength": "Hybrid (specific terms + semantic meaning)",
        },
    ]
